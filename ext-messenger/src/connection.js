...
Connection.prototype._init = function(extPart, name, messageHandler) {
    ...
    // _myExtPart 表示该 npm 包执行所在的part，不同的 part 能获取不同的chrome api
...
// 当 pending 的 resolve 超过一定数量会清除最老的 resolve 函数
...
// sendMessage 触发的时候 cbPromiseResolve 有值，为 resolve 函数。
...
// 将传入的值responseValue封装好之后，调用_postMessage
...
// 将用户的消息内容，消息来源，消息的发送者，回复函数sendResponse交给用户，sendResponse接收到用户传入的值之后，调用_postResponse
...
// 处理接收到的response，通过response中的cdId获取对应的回调函数并执行
...
Connection.prototype._onPortMessageHandler = function(message, fromPort) {
    switch (message.type) {
        // 如果链接建立成功，停止轮询，将初始化之前接收到的消息通过该通道传递给background
